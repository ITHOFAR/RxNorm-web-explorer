...
select
  ...
  c.rxaui,
  c.str,
  (select df.rxcui from rxnrel r join df on df.rxcui = r.rxcui2
...
select
  ...
  (select df.rxcui from rxnrel r join df on df.rxcui = r.rxcui2
   where r.sab = 'RXNORM' and r.rela = 'dose_form_of' and c.rxcui = r.rxcui1) df_rxcui,
  (select bn.rxcui from rxnrel r join bn on bn.rxcui = r.rxcui2
...
select
  ...
  c.rxaui,
  c.str as name,
  (select r.rxcui2 from rxnrel r where r.rxcui1 = c.rxcui and r.rela = 'has_tradename') scd_rxcui,
  (select bn.rxcui from rxnrel r join bn on bn.rxcui = r.rxcui2
   where r.sab = 'RXNORM' and r.rela = 'ingredient_of' and r.rxcui1 = c.rxcui) bn_rxcui,
  (select f.rxcui from rxnrel r join sbdf f on f.rxcui = r.rxcui1
   where r.rxcui2 = c.rxcui and r.sab = 'RXNORM' and r.rela = 'isa') sbdf_rxcui,
  (select sbdc.rxcui from rxnrel r join sbdc on sbdc.rxcui = r.rxcui1
   where r.sab = 'RXNORM' and r.rela = 'consists_of' and r.rxcui2 = c.rxcui) sbdc_rxcui,
  (select psn.str from rxnconso psn where psn.tty = 'PSN' and psn.rxcui = c.rxcui) psn,
  (select s.atv from rxnsat s where s.sab = 'RXNORM' and s.rxcui = c.rxcui and s.atn = 'RXTERM_FORM') rxterm_form,
  (select r.rxcui2 from rxnrel r where r.sab = 'RXNORM' and r.rela = 'dose_form_of' and r.rxcui1 = c.rxcui) df,
  (select s.atv from rxnsat s where s.sab = 'RXNORM' and s.rxcui = c.rxcui and s.atn = 'RXN_AVAILABLE_STRENGTH') strengths,
  (select s.atv from rxnsat s where s.sab = 'RXNORM' and s.rxcui = c.rxcui and s.atn = 'RXN_QUALITATIVE_DISTINCTION') qual_distinct,
  c.suppress,
  (select s.atv from rxnsat s where s.sab = 'RXNORM' and s.rxcui = c.rxcui and s.atn = 'RXN_QUANTITY') quantity,
  exists (select s.atv from rxnsat s where s.sab = 'RXNORM' and s.rxcui = c.rxcui and s.atn = 'RXN_HUMAN_DRUG') human,
  exists (select s.atv from rxnsat s where s.sab = 'RXNORM' and s.rxcui = c.rxcui and s.atn = 'RXN_VET_DRUG') vet,
  (select sbd2.rxcui
   from rxnrel r
...
select
  ...
  c.rxaui,
  c.str as name,
  (select g.rxcui from rxnrel r join gpck g on g.rxcui = r.rxcui1
...
select
  ...
  (select i.rxcui from rxnrel r join "in" i on i.rxcui = r.rxcui2
   where r.rela = 'ingredient_of' and r.sab = 'RXNORM' and r.rxcui1 = c.rxcui) in_rxcui,
  (select pin.rxcui from rxnrel r join pin on pin.rxcui = r.rxcui2
...
select
  ...
  c.rxaui,
  c.str,
  (select dfg.rxcui from rxnrel r join dfg on dfg.rxcui = r.rxcui2
...
select
  ...
  (select dfg.rxcui from rxnrel r join dfg on dfg.rxcui = r.rxcui2
   where r.sab = 'RXNORM' and r.rela = 'doseformgroup_of' and c.rxcui = r.rxcui1) dfg_rxcui,
  (select r.rxcui2 from rxnrel r
   where r.rxcui1 = c.rxcui and r.sab = 'RXNORM' and r.rela = 'ingredient_of') bn,
  (select scdg.rxcui from rxnrel r join scdg on scdg.rxcui = r.rxcui1
